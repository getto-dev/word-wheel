...
const WordWheel: React.FC<WordWheelProps> = ({ letters, onWordSubmit, onCurrentWordChange, onInteraction }) => {
  ...
  // Track container size for adaptive touch thresholds
  useEffect(() => {
  ...
  const findIndexAtPos = (clientX: number, clientY: number) => {
    ...
    // Adaptive threshold based on container size and letter count
    const threshold = Math.max(14, Math.min(20, (containerSize / letterArray.length) * 0.35));
  ...
  const handleMove = useCallback(
    ...
    [isDragging, selectedIndices, shuffledIndices, letterArray, onCurrentWordChange, containerSize]
  );
  ...
  return (
    <div
      key={letters}
      className="relative w-full max-w-[150px] xs:max-w-[180px] sm:max-w-[200px] md:max-w-[250px] short:lg:max-w-[424px] lg:max-w-[472px] mx-auto select-none touch-none animate-wheel">
      <style>
        {`
          @keyframes wheelPop {
            0% { 
      ...
      >
        ...
        {shuffledIndices.map((letterIdx, i) => {
          ...
          return (
            <div
              key={letterIdx}
              className={`absolute flex items-center justify-center rounded-full transition-opacity duration-150 transform-gpu z-10
                w-9 h-9 xs:w-10 xs:h-10 sm:w-10 sm:h-10 md:w-12 md:h-12 lg:w-[74px] lg:h-[74px]
                text-[20px] xs:text-[24px] sm:text-[24px] md:text-[30px] lg:text-[54px] font-medium
        ...
        <button
          ...
          className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 p-2.5 xs:p-3 sm:p-3 md:p-4 lg:p-6 aspect-square rounded-full bg-white hover:bg-white/75 transition-all active:scale-90 z-20"
          aria-label="Перемешать буквы"
        >
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22 16" fill="none" className="w-3 h-3 xs:w-3 xs:h-3 sm:w-3 sm:h-3 md:w-4 md:h-4 lg:w-6 lg:h-6 text-black">
